...
#[test]
fn test_long_decimals_market_integration() {
    // *********************************************************************************************
    // *                              SETUP                                                        *
    // *********************************************************************************************
    let (
    ...
    IERC20Dispatcher { contract_address: market.short_token }.mint(caller_address, 49999999999999999000000); // 49.999 UDC
    ...
    start_prank(market.long_token, caller_address);
    IERC20Dispatcher { contract_address: market.long_token }
        .transfer(deposit_vault.contract_address, 1000000000000000000); // 1 ETH

    start_prank(market.short_token, caller_address);
    IERC20Dispatcher { contract_address: market.short_token }
        .transfer(deposit_vault.contract_address, 5000000000000000000000); // 5000 USDC
    'make transfer'.print();
    // Create Deposit
    ...
    assert(first_deposit.initial_long_token == market.long_token, 'Wrong initial long token');
    assert(
        first_deposit.initial_long_token_amount == 1000000000000000000, 'Wrong initial long token amount'
    );
    assert(
        first_deposit.initial_short_token_amount == 5000000000000000000000, 'Wrong init short token amount'
    );
    ...
    pool_value_info.long_token_amount.print(); // 1 000000000000000000
    pool_value_info.short_token_amount.print(); // 5000 000000000000000000
    ...
    data_store.set_u256(key_open_interest, 1);
    let max_key_open_interest = keys::max_open_interest_key(market.market_token, true);
    data_store.set_u256(max_key_open_interest, 1000000000000000000000000); // 1 000 000
