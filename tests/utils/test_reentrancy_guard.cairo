...
fn test_reentrancy_values() {
    ...
    let entrant: bool =  data_store.get_bool('REENTRANCY_GUARD_STATUS').unwrap() ; // We don't really need to use match, unwrap() should work but however let's keep the same way.
    assert(entrant, 'Entered value wrong'); // Value should be true.

    non_reentrant_after(data_store); // This should set value false.

    // Gets final value
    let after: bool =  data_store.get_bool('REENTRANCY_GUARD_STATUS').unwrap() ; // We don't really need to use match, unwrap() should work but however let's keep the same way.
